...
parm[, "Tmincoeff2"] = qnorm(parm_quant[, "Tmincoeff2"], mean=-.00846, sd=.0846/pvar)
parm[, "Pcoeff1"] = qnorm(parm_quant[, "Pcoeff1"], mean=-.00846, sd=.0846/pvar)
parm[, "Pcoeff2"] = qnorm(parm_quant[, "Pcoeff2"], mean=-.00846, sd=.0846/pvar)
parm[, "intercep"] = qnorm(parm_quant[, "intercep"], mean=-.00846, sd=.0846/pvar)
...
yield = parm |> pmap(compute_almond_yield, clim=clim)
...
yields_df = yields |> map_dfr(`[`, c("maxyield", "minyield", "meanyield"))
```
...
g = yields_df |> gather(value="value", key="yield")
ggplot(g, aes(yield, value, col=yield))+
  geom_boxplot()+
  labs(y="Yield (anomaly)")
...
# will show: how majority of data is distributed. similar info to violin plot
ggplot(yields_df, aes(maxyield))+
  stat_ecdf()
...
tmp_longer = tmp |> gather(maxyield, minyield, meanyield, value="yvalue", key="yieldtype")

# gather across parameters (but not including yields)
tmp_even_longer_lol = tmp_longer |> gather(-yvalue, -yieldtype, value="parmvalue", key="parm")

# now plot using facets for each yield type, parameter combination
ggplot(tmp_even_longer_lol, aes(parmvalue, yvalue, col=yieldtype))+
  geom_point()+
  facet_wrap(~yieldtype*parm,)
```
